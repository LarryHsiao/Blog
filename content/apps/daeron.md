...
於 GitHub Release 頁面下載 `daeron-setup-<version>.exe` 並執行。UAC 會跳出一次 — 將自簽憑證放入 `LocalMachine\TrustedPeople` 需要管理員權限 — 接著安裝程式信任憑證、註冊隨附的 Windows App Runtime 1.6 框架、再註冊 Daeron MSIX。完成後，Daeron 出現於開始功能表並常駐系統匣。

解除安裝請至 **應用程式與功能** → **Daeron** → **解除安裝**，與其他 Windows 應用一致。

需求：Windows 10 build 19041 或更新，x64。

### 手動安裝（備援）

若不願執行安裝程式，同一份 Release 亦提供 `Daeron-<version>-x64.zip` — 安裝程式所包裹的原始 MSIX 套件。兩個步驟：
