+++
title = "蕭富云"
path = "about/jp"
template = "about.html"
+++

<figure>
<img src="https://www.gravatar.com/avatar/866967586e7dcc81d667689948de277c?s=150" alt="蕭富云" />
</figure>

📱 モバイルアプリ開発者｜Android & Flutter｜ヘルスケアテック

本当に良いテクノロジーは、静かに人の暮らしを支えるものだと信じています。

Android と Flutter によるクロスプラットフォーム開発を専門とし、遠隔医療や Bluetooth 医療機器連携など、ヘルスケア領域に深く携わっています。北米・日本・台湾市場にまたがるプロジェクトで、安定性・ローカライズ・そしてユーザーが本当に使えるかどうかを大切にしています。

コードはアウトプットではなく、自分が選んだ使命です。

仕事を離れると、サーキットに集中を 🏎️（アマチュアラリードライバー）、メロディに感情を 🎸（ギタリスト）、そして雪の中に自由を置いてきます 🏂（スノーボード）。

- [larryhsiao@larryhsiao.com](mailto:larryhsiao@larryhsiao.com)
- [LinkedIn](https://www.linkedin.com/in/larryhsiao/)
- [GitHub](https://github.com/larryhsiao)
